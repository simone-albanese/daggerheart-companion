...
| **Difetti 2 + 4** — shell | Il centro del tab GM morto **da 720 a 828**, ultimo pixel coperto a **856**, riga flex sovra-sottoscritta fino a **864** (`Header.tsx:23-24`); un nome più largo di ~118px spingeva SETTINGS fuori schermo | `minmax(0,1fr)` da solo **non** basta: serve anche `min-width: 0`, o il minimo content-based dell'header sborda la traccia già limitata. ~~«occluso al 100% fino a 802px»~~ — **non dichiarato, ritirato**: vedi §5. Le soglie non sono la stessa misura — vedi §5 |
...
4. **La portata di ROLL.** Il reflow lo allontana di **79px** dal bordo
   ...
   stessa cosa: il bordo inferiore di ROLL passa da 385 a **306** di colonna
   utile (identico alle due larghezze), la sua riga passa da 414-480 a
