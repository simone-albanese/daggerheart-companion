      ...
      <main className="stack" style={{ minHeight: 0, overflow: 'hidden' }}>
        ...
        */}
        ...
        {/*
          ...
          hairline. `GmBar` - which declares the same three terms this bar
          does, 60px of buttons over `padding: 0 0 34px` and a 1px `border-top`
          - measures **95.00** at 393x852 with a 34px home-indicator inset.
          This bar was not itself measured; it is 95.00 by the same
          declarations, which is an inference and is written as one. What
          stacking would actually cost the list is less than either figure
          anyway, because `env(safe-area-inset-bottom)` is paid once per screen
          and `GmBar` is already paying it. That has not been measured either,
          and the argument does not need it: the point is that a second bar
          costs a band of this order, not that it costs exactly 95.
