...
import { holdsFavor } from '../../engine/character.ts';
 ...
 * ## Who sees it
 *
 * `holdsFavor`, which asks the dataset for a class feature named `~favor` and
 * asks it of both the class and the multiclass. Not `classRef === 'warlock'`,
 * for the reasons written over `grantsFavor`; not `favor.max > 0`, because
 * every one of the thirteen classes carries the track and that test would
 * answer yes for all of them. A Bard is never offered this, on any roll, at any
 * table.
 *
...
export function FavorRow({ result, hopeGained, layout }: FavorRowProps): React.JSX.Element | null {
  ...
  if (!holdsFavor(character, index)) return null;
   ...
   * belt to `boundCounters`' braces: this is the one place in the app that adds
   * a Favor, and `favor.max` is what the store and the codec both clamp to.
   */
  const take = (): void => {
    setTaken(true);
    update((c) => ({
