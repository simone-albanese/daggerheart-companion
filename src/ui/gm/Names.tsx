 ...
 * is **367.00** - the one figure here that was measured, and it was measured
 * in the sibling tool that shares this sheet (`Reference.tsx`). The vertical
 * side is untouched by that pass: 797 and 753 both spend the panel's own
 * border nowhere, and 753 spends the title row's rule nowhere either, so both
 * are wrong by some small number of pixels that NOBODY HAS COUNTED. No
 * replacement is offered for them here, because deriving one on paper is the
 * exact move this pass exists to undo. 704, 984 and 1060 lose 2 apiece by the
 * same border as 369 did, and that much is implied by the measured 367.00
 * without being measured itself. Each number below that starts from 369 - the
