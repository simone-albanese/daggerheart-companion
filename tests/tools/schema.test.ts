...
describe('the 5 -> 6 converter', () => {
  it('changes no character field, and says so in its note', () => {
    ...
    // Four notes, not one: `migrateCharacterRecord` walks to the CURRENT
    // schema, which is 9, so the 6 -> 7, 7 -> 8 and 8 -> 9 steps run behind
    // this one. Asserting the whole list rather than the first entry is what
    // keeps the step this describe block is about identifiable after later
    // bumps.
    ...
    const { transformationRef, stanceRefs, focus, favor, ...rest } = after.record;
    ...
    /*
     * ZERO, not the three a brand-new sheet gets. A converter runs on somebody
     * already playing, and the split is the owner's decision - the argument is
     * on `SCHEMA_VERSION` in `shared/types.ts`. Pinned here because the two
     * numbers are three lines apart in two different files, and a later hand
     * that "tidies" them into agreement breaks the one that matters.
     */
    expect(favor, 'seeded by the 8 -> 9 step, and seeded EMPTY').toEqual({ marked: 0, max: 6 });
