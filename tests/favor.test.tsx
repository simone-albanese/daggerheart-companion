...
import { COUNTER_CEILINGS, grantsFavor, holdsFavor, indexDataset, newCharacter } from '../src/engine/character.ts';
...
describe('holdsFavor: the live question, with the multiclass arm on it', () => {
  it('reads BOTH slots, which is the whole difference from the seed', () => {
    ...
    expect(holdsFavor(pact, LAYER), 'and still holds the feature').toBe(true);
    // The first slot, which both predicates agree on.
    expect(holdsFavor(newWarlock(), LAYER)).toBe(true);
    // And neither slot.
    expect(holdsFavor(newCharacter({ classRef: 'plain' }, LAYER), LAYER)).toBe(false);
  });
  ...
  it('agrees with the feature list the app already prints, on every class in the book', () => {
     ...
     * `characterFeatures` lists, `holdsFavor` answers. Change one and this goes
    ...
    for (const [classRef, multiclassRef] of [
      ...
      expect([classRef, multiclassRef, holdsFavor(c, LAYER)]).toEqual([
        classRef,
        multiclassRef,
        named(c),
      ]);
  ...
  it('says no to a ref the index cannot name, rather than throwing', () => {
    ...
    expect(holdsFavor(c, LAYER)).toBe(false);
    expect(holdsFavor({ ...c, classRef: '', multiclassRef: null }, LAYER)).toBe(false);
  });
  ...
  it.skipIf(!hasDataset())('is the Warlock and only the Warlock, over the real book', () => {
    ...
    const held = dataset.classes.filter((k) =>
      holdsFavor(newCharacter({ classRef: k.id }, ix), ix),
    );
    ...
    for (const k of dataset.classes) {
      expect([
        k.id,
        holdsFavor(newCharacter({ classRef: k.id, multiclassRef: 'warlock' }, ix), ix),
      ]).toEqual([k.id, true]);
    }
  });
});
