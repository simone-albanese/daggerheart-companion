 ...
 *
 * One file used to care: the Core Rulebook importer's pdf.js worker was held
 * back from the precache and refetched here, at the last moment a network was
 * certain, so that a device which took an update in a tunnel did not come out
 * of it with an importer that had silently stopped existing. The importer, the
 * worker chunk and the dependency have all been removed, so nothing is held
 * back any more and install is precache and nothing else.
 *
 * The copy already sitting in the asset caches of devices that imported before
 * the removal is not orphaned by that. `pruneAssets` deletes every hashed file
 * the new document does not name, and the new document names no import worker,
 * so the first activation of a build without it is also what takes those 1.6 MB
 * off the device.
 */
...
async function takeOver() {
  ...
  // The refill goes ahead of the delete, so that no ordering here can read as
  // "drop the copy we have and then go looking".
  await pruneAssets();
...
/**
 * `hello` is a client saying it has loaded, and it exists for one reason.
 *
 * A browser can reclaim Cache Storage at any time, and it does not install a
 * new worker afterwards - a replacement installs only when the bytes of this
 * file change. So an activated worker can be left sitting on nothing with
 * `activate`, the only other thing that calls `ensurePrecached`, never coming
 * round again. A page that has just loaded is not yet controlled by that worker
 * either, so its navigation did not pass through here and cannot have refilled
 * anything.
 *
 * This message used to be `warm-importer`, posted by a desktop that had just
 * decided it could run the Core Rulebook importer, and rebuilding the precache
 * was the first thing it did before fetching the importer's chunk. That is what
 * caught the production case: one registration, activated, a `dhc-shell-v1`
 * holding nothing, and an app one flight from a white screen. The importer is
 * gone and the chunk with it; the reason the worker needs to hear from a client
 * at all did not go anywhere, so the message stays and now says only what it
 * was always really for.
 */
self.addEventListener('message', (event) => {
  if (!event.data) return;
  if (event.data.type === 'skip-waiting') self.skipWaiting();
  if (event.data.type === 'hello') event.waitUntil(ensurePrecached());
});
...
async function warmAssets(html) {
  ...
  const { shell } = await reachableFrom(html, assets, (url) => tryFill(assets, url.href));
 ...
 * here and goes with the superseded. There is no longer any chunk deliberately
 * left uncached - the importer's pdf.js worker was the one, and it is gone - so
 * nothing is currently hiding behind that edge; put something back behind it
 * and this is where it would quietly stop existing.
 *
 * That same edge is what removes the importer from devices which cached it
 * before it was deleted. Its worker is not named by any document this build
 * ships, so it is never in `hashed`, and the first activation after the update
 * sweeps it out of the asset cache with the rest of the superseded.
 */
