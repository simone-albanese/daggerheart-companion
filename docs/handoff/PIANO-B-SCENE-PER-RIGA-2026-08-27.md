...
>
> The plan inherited the wrong range honestly: `src/ui/gm/gmStore.ts:200` makes the same `53-58` citation, inside `runScene`'s own interface docblock (`:196‑206` at `ab66cf2`; `grep -n '53-58' src/ui/gm/gmStore.ts` returns that one line and no other — cite the docblock range rather than the line if you want a citation that cannot drift by one). If a citation is wanted for the rebuild-by-spread discipline, the writers are `gmStore.ts:983` (`i.kind === 'scene' && i.id === s.liveScene ? { ...i, combatants: [] } : i`) and `gmStore.ts:1061`. **Do not copy this sentence verbatim into a brand-new docblock** — a false file:line inside fresh prose is the defect this repo marks rather than re-derives.
...
> - It is **93 lines**, not *"~55"*, plus **12** more for the interface docblock and declaration at **`gmStore.ts:196‑207`**: **105 lines** in all. This sentence promises *"with their docblocks"* but cites one only for `adoptBoard`, so `runScene`'s twelve lines have to be added to the deletion by hand. **Do not reach for `:226` for them.** `grep -n 'resume' src/ui/gm/gmStore.ts` returns exactly two lines, `:226` and `:1081`, and `:226` sits ten lines inside *`adoptBoard`*'s docblock (`:208‑230`): the resume invariant is named there and goes out with `adoptBoard` — which is why the struck bullet two below this one puts `adoptBoard`'s docblock at `:208‑231` and not `:196‑207`. `runScene`'s own docblock (`:196‑206`) never uses the word — it opens *"The one action in `gmStore` that sets a combatant list wholesale"*. (`CORR-04`)
...
> **RAGIONE STANTIA — 30 ago 2026 · `CORR-29`, `HAZ-56`.** The lane committed — as PR #51 — so there is no working tree left to resolve this in, and both citations have drifted: at `ab66cf2` the two guards are `RuleSearch.tsx:1661` (`{moment !== null && askedBand}`) and `:1617` (`{!browsing && askedBand}`) (`git show ab66cf2:src/ui/shared/RuleSearch.tsx | grep -n 'askedBand}'` returns those two lines and no others). **The loose end itself is real and still open** — the two guards do still differ on `main` — but it is now a debt on shipped code rather than a chore before a commit, and `HAZ-56` is why it stays harmless.
