...
**Everything below this block was written against `14c995a` and has now been measured against the tree it would actually run on.** Fourteen areas, one per section of this document, 224 agents; every verdict of FALSE was then handed to three independent skeptics whose job was to refute it, and only what survived a **majority** of the three is written in (MEMORY: always refute your own corrections). *Majority, and not unanimity.* Over the source object's 34 `standingCorrections`, `collections.Counter(x['refuteVotes'] for x in d['standingCorrections'])` returns `{'0/3': 25, '1/3': 9}` — **nine of the thirty-four were written in over one refute vote**, and they are `CORR-03`, `CORR-05`, `CORR-18`, `CORR-20`, `CORR-22`, `CORR-23`, `CORR-28`, `CORR-30`, `CORR-31`. Weigh those nine yourself before building on them, and weigh them by the source object's own `teeth` field rather than by the order they are listed in: **five of the nine are `changes-the-build`** — `CORR-03`, `CORR-05`, `CORR-28`, `CORR-30`, `CORR-31` — and the first two are the ones standing between an executor and a literal `delete` of the wrong lines of `gmStore.ts`. The other four (`CORR-18`, `CORR-20`, `CORR-22`, `CORR-23`) are `cosmetic`, though `CORR-23` carries more downstream figures than any other marker. The shape of the result:
...
- **36 of 70 challenges were themselves overturned.** That is the number that makes the other 34 worth trusting: the refutation round threw out more than half of what the verifiers called wrong. What is marked below is what survived being argued against by three skeptics — 25 of the 34 unanimously, 9 over a single refute vote (the nine are listed above) — and not what somebody disliked on a first reading. **Refuting is not free either.** A later pass re-read this verification and found kills that did not survive contact with the file; that is why `CORR-05`'s docblock half is struck below, and it is why the marker below `CORR-05` is a withdrawal rather than a correction. The four counting figures in this block do reproduce exactly from the source object: `trueClaimCounts` sums to **251**, `standingCorrections` is **34**, `newHazards` is **64**, `overturnedVerifierFindings` is **36**, and 34 + 36 = **70**.
...
| `tests/store/campaignSchema.test.ts` | A3 inserted a 342-line block at **`:741`**, after old `:740` (`git diff -U0 3aa8f23..ab66cf2 -- tests/store/campaignSchema.test.ts \| grep '^@@'` → `@@ -67 +67 @@` and `@@ -740,0 +741,342 @@`; the first hunk swaps one line for one and shifts nothing). Citations **above** it land (`:112‑120`, `:136`, `:144`, `:160‑163`, `:192`, `:210`, `:628‑660`, `:645`); citations **below** are **+342** (`:891`→`:1233`, `:899`→`:1241`, `:896`→`:1238`). |
...
> **RAGIONE STANTIA — 30 ago 2026 · rinvia a `CORR-29`, `CORR-30`, `CORR-31`.** Every figure in the paragraph above was exact when it was written, against `14c995a`, and every one of them is now history: **Lane S has landed in full.** `RuleSearch.tsx` and `Search.tsx` shipped in `38181de`, merged as `f0af890`, PR #51 — so *"uncommitted work in progress"* describes a working tree that no longer exists, and the closing *"a third of it is already shipped"* understates it: **all of it is**. The one count that moved with it: `tests/ui/searchScreen.test.tsx` was **8** anchored `it(` at `14c995a` and is **21** at `ab66cf2` (`git show ab66cf2:tests/ui/searchScreen.test.tsx | grep -cE '^[[:space:]]*it\('` → 21). Read §5 with `CORR-29`…`CORR-31` and `HAZ-64` beside it — including its two headings, which still command work that shipped.
...
> **CORRETTO — 27 ago 2026 · CORR-23.** *"the whole suite has **3361** `it(` blocks"* was six too high the day it was written, and the suite has grown twice since. **Take the method, not the total.** The plan's `grep -o '\bit('` counts `it(` wherever it appears — inside comments **and inside string literals** — while the anchored `grep -cE '^[[:space:]]*it\('` does not. At `ab66cf2` the six extras are **three comments** (`gmShell.test.tsx:408`, `sceneTruth.test.tsx:20`, `domainCardView.test.ts:316`) and **three string literals** in the harness sweep (`sweep.test.ts:204`, `:326`, `:327`), so a lane told to look for comments will not find half of them: `xargs grep -n '\bit(' | grep -vE ':[0-9]+:[[:space:]]*it\('` lists all six. Take the file list off a **tree** and not the index — `git ls-tree -r --name-only <commit> | grep -E '\.test\.(ts|tsx)$'`, because `git ls-files` reads the index and so can only ever produce the checkout's own leg — and the two return **3361 / 3355** at `14c995a`, **3508 / 3502** at `3aa8f23`, and **3542 / 3536** at `ab66cf2`. The over-count is 6 at all three commits — only the totals move, so re-derive with the anchored form and never inherit the figure. Add the 39 `it.each(` sites (same file list, `xargs grep -o 'it\.each(' | wc -l`, at `ab66cf2`) for the total number of declarations. The file counts in this bullet have moved too — *"25 in the union"* is **27** at `3aa8f23` and **28** at `ab66cf2`, after Wave A added `tests/fixtures/factories.test.ts` (see `CORR-27` at Wave B's Owns block). The suite itself went **154** (`14c995a`) → **161** (`3aa8f23`) → **162** (`ab66cf2`) (`git ls-tree -r --name-only <commit> | grep -cE '\.test\.(ts|tsx)$'`), and **the first leg is #52 alone, inside one calendar day**. `git diff --name-status 14c995a..3aa8f23 -- tests | grep '^A'` returns seven `.test.*` files (plus one fixture), and each one's `--diff-filter=A` commit is an ancestor of `b0cb250` (PR #52) and not of `b0cb250^1`. **#51 contributed none over this baseline**, because `14c995a` is itself a commit on #51's branch — the file #51 brought is already inside the 154, and `git diff --name-status 14c995a..f0af890 -- tests | grep '^A'` is empty. Nor is it three days: both endpoints are `2026-08-27` (`12:15:47` → `16:30:27`), and `git log --format='%ai' 14c995a..3aa8f23 | cut -d' ' -f1 | sort -u` returns that one date for all 36 commits — four and a quarter hours.
>
...
>
> The plan inherited the wrong range honestly: `src/ui/gm/gmStore.ts:200` makes the same `53-58` citation, inside `runScene`'s own interface docblock (`:196‑206` at `ab66cf2`; `grep -n '53-58' src/ui/gm/gmStore.ts` returns that one line and no other — cite the docblock range rather than the line if you want a citation that cannot drift by one). If a citation is wanted for the rebuild-by-spread discipline, **do not take one out of `gmStore.ts`**: the two spreads there live inside `clearScene`, which B3 re-signs, and inside `runScene`'s park-and-resume `.map`, which B3 deletes outright — so a docblock written in this lane would ship citing code the same atomic commit removes. After B3 the only wholesale writer is `withSceneFight`, declared in §1 above: **cite it by name and give it no line number at all.** **Do not copy this sentence verbatim into a brand-new docblock** — a false file:line inside fresh prose is the defect this repo marks rather than re-derives.
...
> - It is **93 lines**, not *"~55"*, plus **12** more for the interface docblock and declaration at **`gmStore.ts:196‑207`**: **105 lines** in all. This sentence promises *"with their docblocks"* but cites one only for `adoptBoard`, so `runScene`'s twelve lines have to be added to the deletion by hand. **Do not reach for `:226` for them.** `grep -n 'resume' src/ui/gm/gmStore.ts` returns exactly two lines, `:226` and `:1081`, and `:226` sits inside *`adoptBoard`*'s docblock and not inside `runScene`'s — one object, named once: the comment is `:208‑230` and the declaration under it is `:231`. The resume invariant is named there and goes out with `adoptBoard`. `runScene`'s own docblock (`:196‑206`) never uses the word; the sentence quoted for it here — *"The one action in `gmStore` that sets a combatant list wholesale"* — is that docblock's **second** paragraph and not its opening. (`CORR-04`)
> - **`adoptBoard` is `gmStore.ts:990‑998`** (9 lines, closing brace included), not `913‑920`. Deleting `913‑920` today mangles the initial state object and `addToRoster`. (`CORR-05`, STALE_REASON.)
> - **Struck, 30 ago 2026 — the bullet above also read *"and its interface docblock is `:208‑231`, not `206‑229`"*, and that half is withdrawn.** `206‑229` is **exact at the plan's own declared base**: `git show 14c995a:src/ui/gm/gmStore.ts | sed -n '206,229p'` opens on `/**` at `:206`, closes `*/` at `:228` and declares `adoptBoard: (sceneId: string) => void;` at `:229`. It is also not a correction this verification ever made — the strings `206-229` and `208-231` occur in the source object **only** under `overturnedVerifierFindings` (entries 0, 1, 2 and 26), all four at **3/3**, on exactly this ground; `standingCorrections[4]`, which *is* `CORR-05`, says nothing about a docblock at all. The `+2` is drift, and it is already recorded as drift, in §0's re-anchoring table: *"the interface block drifted **+2** (`runScene:` 205→**207**, `adoptBoard:` 229→**231**)"*. **A correction filed against a claim the plan got right is the disease this document exists to cure**, and this marker block committed it once; the bullet above it separately mis-assigned `:226`, which is the mirror failure — a citation invented in fresh prose. Both were introduced by the marking pass, not by the plan.
>
...
>
> The *"never regenerated"* argument lives in the **test file**, where the others put it: `campaignSchema.test.ts:15‑16`, `:142‑143` (*"A fixture rewritten by a later build proves only that the current code can read its own output"*) and `:160‑163` — all three above A3's inserted block and therefore still landing (§0's re-anchoring table carries that block's line, and is the only place in this document that should). **Following this line literally puts a comment block into a `.json` file**, `JSON.parse` throws inside `readFixture` (`:72‑76`), and the whole `the committed fixtures` describe goes red. Write the argument in the test, not in the fixture.
...
> **CORRETTO — 27 ago 2026 · CORR-14.** **Local `main` is not `e25db1f`.** Run both of these in your own worktree — `git rev-parse --short main`, and `git rev-list --count e25db1f..main`. **Their two answers are struck from this marker rather than re-derived into it**, because the pair written here on 27 ago was already wrong in both halves by 30 ago, which is the marker's own point. A worktree cut at `e25db1f` has neither #51 (the search topic index), nor #52 (campaign import + backup), nor #56 (Wave A itself).
>
...
> **RAGIONE STANTIA — 30 ago 2026 · rinvia a `CORR-29`, `CORR-30`, `CORR-31`, `HAZ-64`, `HAZ-61`.** **This lane has shipped in full**, and the block below is a record of what was asked for, not an assignment. §0 says so at the top of this document and §5's two headings carry the same marker for the same reason; this is the third place, and the only one a scheduler reads lane assignments from. Two cells must not be carried into a wave plan: the **Owns** list, which hands out three files that are already on `main` (the branch named in the next paragraph no longer exists), and the **Acceptance** cell, whose *"a diff in either is the signal the lane grew past its own boundary"* contradicts Wave B's own grep-to-zero gate over `tests/gm/ruleSearch.test.tsx` — the two acceptance criteria fight over one file, which is `HAZ-61`.
...
- `grep -rni "the board is running another scene" src shared` returns nothing, and the only file under `tests` that names the string is the absence test that forbids it.

  **Corrected 2026-08-31, during B8's repair pass.** This bullet said *no source or test file anywhere in the tree* contains the string, and it fails the same way the bullet below it did: the plan mandates the thing that makes it red. §6 test 28 is an ABSENCE test — it walks every state two scene rows, one encounter row and a fight can reach and asserts that none of them puts the sentence on the glass — and a list of forbidden strings has to spell the forbidden strings. Measured on `wave-b` at B8: `grep -rni "the board is running another scene" src shared` exits 1 with no output, and the same grep over `tests` returns four lines, all in `tests/gm/sessionList.test.tsx` and all inside that one test — its title, its docblock naming the four `<Fact>` sentences that used to say it, and the two casings in its `FORBIDDEN` array. Any of the four can be deleted to turn the old gate green, and deleting them is deleting the only thing in the tree that checks the deliverable. So the gate runs on `src shared`, and `tests` is read rather than grepped. (No line numbers here on purpose: C1 is scheduled to edit that file next.)
...
- `grep -rn "adoptBoard\|runScene" src shared tests` returns nothing **outside the five named survivors below**.
  ...
  **Corrected 2026-08-31, during B8's repair pass — survivor 5 is a class, and one grep not to substitute for this bullet.** Survivor 5 says *prose naming a schema-4 field or verb*; the files after the dash are examples, and reading them as the exhaustive list turns this gate red on prose it was written to protect. Measured on `wave-b` at B8, `grep -rn "adoptBoard\|runScene" src shared tests` returns 14 lines: nine in files survivor 5 names, and five in four files it does not — `tests/gm/gmStore.test.ts`, `tests/gm/sceneClocks.test.tsx`, `tests/store/campaignRoundTrip.test.ts` (two lines) and `tests/engine/encounter.test.ts`. Every one of the five is a `*` comment line saying what the schema-4 verb used to do, so that a reader knows what the test underneath stopped needing; they are survivor 5 exactly, and the only way to make a file-list reading go green is to delete the arguments this wave exists to leave behind. One correction the other way: `src/store/campaignImport.ts`, named in survivor 5, now matches neither this grep nor `liveScene` — B5 rewrote it. An over-named survivor costs nothing, but do not read it as a promise that a hit is there.

  And do not replace this bullet with a narrower pattern plus exclusions. B8 proposed `grep -rn "liveScene[^s]\|adoptBoard\|runScene" src shared tests` minus the 4 → 5 converter, the frozen v4 fixtures and `not.toHaveProperty`; run verbatim on `wave-b` at B8 that returns eight lines, headed by `src/store/campaignMigration.ts`'s mandated `liveScene: null` — the line survivor 4 calls the load-bearing one, and whose own comment records that deleting it leaves `npx vitest run tests/store/` green. A gate that hands the next executor a red list with that line at the top is worse than no gate at all. This bullet, with its five survivors read as classes, is the one to run.
...
- **`HAZ-45`** **An untracked probe file is sitting in the tree**: `tools/.cache/dbg.test.ts` (25 lines, `describe('dbg', …)`, stubs `localStorage` and calls `backupStatus`). Anyone recounting the suite with a bare `find` picks this file up, and so matches neither `CORR-23`'s tracked figure — taken off `git ls-tree`, which cannot see an untracked file — nor a vitest run, which does not collect it. **Two different counts, and this hazard deliberately names neither**, so that the integer here can never be mistaken for the integer there. Exactly the leftover MEMORY warns about — *check the tree for probe files an audit left behind.*
...
- **`HAZ-63`** §3's cut point `e25db1f` is **dozens of commits behind `main`, and falling further every day** — count it yourself, see `CORR-14`. Branching there rebuilds on a tree missing `src/store/campaignImport.ts`, `src/store/backup.ts`'s campaign leg, `TakeIn.tsx`, the shipped search index, and all of Wave A, and then meets every token-bearing file above as a merge surprise.
...
> **NOTA DI FUSIONE — 31 ago 2026.** Le due correzioni qui sotto sono la stessa scoperta fatta
> due volte e in modo indipendente: la passata di verifica del 27 agosto (`CORR-17`) leggendo il
> blocco, e il passo B6 il 31 agosto riscrivendolo. Sono tenute entrambe perché **la seconda
> contiene una falsità che la prima non aveva trovato** — il punto 2, sull'ordine della striscia.
> Le citazioni `SessionBody.tsx:531` / `:560` / `:521‑583` dentro `CORR-17` sono precedenti alla
> riscrittura di quel file in `11348fc` e **non vanno ri-derivate**: ri-ancorare per stringa
> (`grep -n 'TAKE THE FIGHT ON THE BOARD'` non restituisce più nulla — è proprio il punto).
...
>
> Each sat in a 10px gap, so the most a scene row could lose is one paragraph plus 10. Against that, the two unconditional sentences grew: the opener 31.72 → 47.58 (2 righe → 3) and *To change this plan* 63.44 → 95.16 (4 → 6), at **both** sizes; *Parked here* → *In this scene* is 47.58 either way. Net **+47.58px per scene row, in every state**. The arm: **667.73 → 715.31** holding a fight, **510.16 → 557.73** planned. The three-row list: **1802.39 → 1897.54, +95.15px**.
>
...
>
> **260.00px of reach at both sizes, and 52.00px of row with it.** The 52.00 is Wave B's own doing: `OPEN THIS FIGHT` is 156.20 where `BACK TO THIS FIGHT` was 178.67, so it pairs with `CLEAR THIS FIGHT` (148.63) — 156.20 + 8 + 148.63 = **312.83**, inside 349.00 and inside 331.00, where the old label needed 335.30 and wrapped at 375×667. On a row with no fight there is no `CLEAR` to pair with: the strip stays 252.00 and the reorder buys reach only, 230.00 → 22.00. The third arrangement puts the primary in the same place and costs the 52.00 back, and it puts the arm's one destructive verb at the top of the strip. **Decision: the primary goes last, `CLEAR THIS FIGHT` stays where it is.** The 8px neighbour is the price, and `CLEAR` arms, so a mis-tap changes a label. Re-measured after the move: the four crossing verbs are **unchanged** (289.06 / 293.89 / 246.41 / 251.23), `CLEAR` 148.63, `OPEN THIS FIGHT` 156.20, `START THIS FIGHT` 162.61, `docOverflowX` 0.00, arm 663.31, list **1845.54** — so Wave B and C1 together cost **+43.15px** over three rows, not +95.15.
>
...
> **RAGIONE STANTIA — 30 ago 2026 · `CORR-30`, `HAZ-64`.** The heading is history: **this tree is built and shipped.** `git show ab66cf2:src/ui/search/Search.tsx | grep -n 'function KindGrid(\|function ChapterRows('` returns `250` and `341`, rendered at `:486` and `:496`, and `:131` already writes down why neither is exported. Read what follows as the record of a design that landed, not as a work list — which is `HAZ-64` stated as a rule.
...
> **RAGIONE STANTIA — 30 ago 2026 · `CORR-31`, `HAZ-64`.** The heading is history: these guards are **written**. `tests/ui/searchScreen.test.tsx` holds **21** anchored `it(` at `ab66cf2` against 8 at `14c995a` (`git show ab66cf2:tests/ui/searchScreen.test.tsx | grep -cE '^[[:space:]]*it\('`), including `:424` *"opens a kind onto its records, under a sticky row that shuts it again"*. Eight of the nine below are covered; guard 8 is half-covered (`CORR-31`). Nothing in this list is owed until it has been re-read against `main`.
...
> **RAGIONE STANTIA — 30 ago 2026 · `CORR-29`, `HAZ-56`.** The lane committed — as PR #51 — so there is no working tree left to resolve this in, and both citations have drifted: at `ab66cf2` the two guards are `RuleSearch.tsx:1661` (`{moment !== null && askedBand}`) and `:1617` (`{!browsing && askedBand}`) (`git show ab66cf2:src/ui/shared/RuleSearch.tsx | grep -n 'askedBand}'` returns those two lines and no others). **The loose end itself is real and still open** — the two guards do still differ on `main` — but it is now a debt on shipped code rather than a chore before a commit, and `HAZ-56` is why the *risk* is dead rather than the loose end: `asked` is hard-nulled under a browse, so the divergence cannot be reached. The divergence is what is still owed.
...
> - **`tests/fixtures/factories.ts` has no board literal.** `grep -n 'board:' tests/fixtures/factories.ts` exits 1 — the file declares no `board` key. Use that narrow form and not the wider `grep -in "board"`, which A2's rewrite of this file turned into a false witness: at `ab66cf2` it exits **0**, on the prose word in the new docblock at `:342`. The board literal is `emptyBoard()` at **`shared/campaigns.ts:670‑678`**, a file Wave B already owns exclusively; its other references are `gmStore.ts:40`/`:326`, `gmScreen.test.tsx:335` and `campaignSchema.test.ts:48`/`:244`/`:628`. **The work is covered; the row points at the wrong file.** The parenthesis is still right — `NO_FIGHT` is the row's shape and stays correct — except for its line, which **matched no commit even when the plan was written**: *"What Wave A actually changed"* gives A2's move as 249 → 251, and the row above says `:252`. Re-anchor it with `grep -n NO_FIGHT tests/fixtures/factories.ts` and never off the number in that row. It also collides in *name* with the constant §1 introduces (`HAZ-02`, and the fifth entry of §3.5). (`CORR-12`)
> - **PIN — 30 ago 2026. The `its` column above is anchored to the plan's own base, `14c995a`, and exactly five of its cells have moved since.** All twenty-four files the `its` **column** counts — the table names one more, `tests/fixtures/factories.ts`, whose `its` cell is empty — were re-measured at both commits with `git show <commit>:<file> | grep -cE '^[[:space:]]*it\('`. **Twenty-three of the twenty-four are exact at `14c995a`.** The twenty-fourth is `sceneTruth`'s **43**, and that one cell is a *method* error (`CORR-25`) and not staleness — `sceneTruth` is **42** by the anchored grep at `14c995a` **and** at `ab66cf2`, so `CORR-25` and `CORR-26` hold at both. The five that moved, `14c995a` → `ab66cf2`: `tests/gm/gmStore.test.ts` **67 → 75**, `tests/gm/sessionList.test.tsx` **91 → 99**, `tests/store/campaignSchema.test.ts` **76 → 88**, `tests/gm/session.test.ts` **34 → 38**, `tests/store/campaignFile.test.ts` **22 → 26**. The other nineteen are unchanged at `ab66cf2` — `sceneTruth` 42, `sceneConfirmation` 8, `sceneDamage` 8, `sceneSwitcher` 11, `sceneClocks` 11, `campaignMigration` 21, `minionGroups` 17, `names` 16, `sendCarry` 10, `countdownArm` 21, `restControl` 26, `merchant` 26, `reference` 52, `ruleSearch` 85, `partySizeDisagreement` 13, `fearOnTheGlass` 6, `encounterBump` 4, `gmScreen` 74, `tests/ui/screens.test.tsx` 9. **B7 sizes itself off this column — re-run the command, never inherit the number** (MEMORY: un numero ereditato va rimisurato). The `88` is the same figure *"What Wave A actually changed"* now pins for `campaignSchema.test.ts`; the two places agree.
>
>   **This pin covers the `its` column and nothing else.** Every line number in the `what happens` column beside it — the describe anchors in `gmStore.test.ts` and `sessionList.test.tsx`, the 44px sweep, `floorsOutsideTheSweep`, `:891`, `:899`, `:185-192`, `:702` — is a `14c995a` number too, and **this pass deliberately did not re-measure one of them**: they point into the twenty-eight test files Wave B is rewriting, so a figure re-derived today is wrong tomorrow. Re-anchor every one of them by its describe or helper **name**, under §0's rule, and never by adding an offset. The same applies to Wave C1's acceptance cell, which cites `floorsOutsideTheSweep` by line: run it by name.
