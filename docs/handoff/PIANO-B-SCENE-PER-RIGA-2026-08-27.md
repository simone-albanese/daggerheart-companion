...
> **CORRETTO — 27 ago 2026 · CORR-24.** *"exactly **12** `setState({… combatants …})` seed lines exist in the suite"* counts only the seeds that fit on one physical line. A brace-balanced scan of every `setState(...)` whose argument contains `combatants` finds **29**, all of them on `useGm.setState`, spread over **17 files**; the identical scan at `14c995a` also returns 29, so this is not staleness. The plan's own grep is reproducible — `grep -rn 'setState({.*combatants' tests --include='*.test.ts' --include='*.test.tsx'` returns 12 in this worktree today — and the seventeen it cannot see are multi-line calls: `countdownArm:96`, `encounterBump:78`, `fearOnTheGlass:176`, `gmScreen:93` and `:258`, `merchant:111`, `minionGroups:91`, `names:103` and `:231`, `partySizeDisagreement:66`, `reference:85`, `ruleSearch:131`, `sceneClocks:49`, `sceneSwitcher:209`, `sendCarry:97`, `sessionList:704` and `:1047`.
>
...
> **CORRETTO — 27 ago 2026 · CORR-23.** *"the whole suite has **3361** `it(` blocks"* is **3502** real ones today, and the figure was six too high the day it was written: the plan's `\bit(` grep counts 6 comment occurrences, so its own method returns 3508 here. Add 39 `it.each(` sites for the total number of declarations. The file counts in this bullet have moved too — *"25 in the union"* is 27, and **28** today after Wave A added `tests/fixtures/factories.test.ts` (see `CORR-27` at Wave B's Owns block). The suite grew **154 → 161** files in three days, under #51 and #52.
>
...
**Wave A is merged: `ab66cf2`, PR #56.** The table above is now a record of what was *asked for*. What shipped is nine commits over fourteen files, **+1514 / −51** (`git log --oneline 3aa8f23..ab66cf2`; `git diff 3aa8f23..ab66cf2 --stat`), and four things in it change what Wave B writes.
...
> - **`tests/fixtures/factories.ts` has no board literal.** `grep -in "board" tests/fixtures/factories.ts` exits 1 — the string does not occur in the file at all, in any case. The board literal is `emptyBoard()` at **`shared/campaigns.ts:670‑678`**, a file Wave B already owns exclusively; its other references are `gmStore.ts:40`/`:326`, `gmScreen.test.tsx:335` and `campaignSchema.test.ts:48`/`:244`/`:628`. **The work is covered; the row points at the wrong file.** The parenthesis is still right — `NO_FIGHT` is the row's shape and stays correct — except that A2 moved it to **`:251`**, and it collides in *name* with the constant §1 introduces (`HAZ-02`, and the fifth entry of §3.5). (`CORR-12`)
