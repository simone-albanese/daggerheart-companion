...
> **REFUTED 2026-08-27, and the code no longer rests on this argument.** `addCampaign` answers about the DISK; `state.campaigns` is memory, and the two part the moment a record leaves the disk without this tab noticing (a second tab's REMOVE, or storage eviction). The landed id could therefore be exactly the id the board was open on — the restore landed under it, `switchCampaign` early-returned, and the next flush wrote the stale board over the record just restored. `applyCampaignImport` now gates the key against memory as well as the disk, and `TakeIn`'s step 7 filters the landed id out of the list before prepending, so the conclusion holds again but for a different reason. Pinned by "never offers an id the list in memory holds, even when the disk has lost it" (`tests/store/campaignImport.test.ts`) and "restores it beside the open board instead of under it" (`tests/gm/gmSave.test.tsx`). The `gmStore.ts:1237` citations below are also stale; read the function, not the line.
...
> **The two bullets below rest on the argument refuted in §1 above.** Both conclusions still hold, on the memory gate rather than on add-only.
...
- `App.tsx:399` — the heading condition becomes `integrity.missingIds.length + integrity.missingCampaignIds.length > 0`. Today a device that lost only its campaigns is headed "THE LIBRARY DID NOT OPEN", which names the wrong problem. `integrity.message` is already rendered verbatim at `:402`, so the campaign sentence needs no JSX. One conditional chip beside RESTORE FROM A BACKUP, drawn whenever campaigns are missing: `OPEN THE GM TOOLS` → `setScreen('gm')`, because the campaign restore lives behind SAVE inside that section and the shell must not grow a second copy of it.

**Two corrections, both shipped.** The heading needs three arms, not two: a campaign store that will not open leaves *both* id counts at zero on purpose, so it still fell to the wrong one - `IntegrityReport` now carries `readable`/`campaignsReadable` and the middle arm reads THE CAMPAIGNS DID NOT OPEN. And the chip is **not** behind `prefs.gmSection`: a GM who switched the section off still has every campaign (Settings promises exactly that), and hiding the chip left them the restore chip alone, which lands on an import that throws a `.dhcampaign` back. It turns the section on and says so in its own label instead.
