...
3. **SAVE A COPY becomes a backup the app can see.** §3h records it, so an iOS GM who exports each campaign into Files/iCloud reads *"A copy of "The Sablewood Winter" was saved from this device 3 days ago, through the share sheet. The app cannot check where it went."* instead of "no backup yet". And §2 makes that copy readable again, which is the half that was missing.
